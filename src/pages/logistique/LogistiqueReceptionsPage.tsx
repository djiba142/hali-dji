...
import { 
  Package, Plus, Search, Filter, Calendar, Ship, 
  ArrowRight, CheckCircle2, AlertTriangle, Warehouse,
...
export default function LogistiqueReceptionsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (newReception: any) => {
      const { error } = await (supabase as any)
        .from('logistique_receptions')
        .insert(newReception);
      if (error) throw error;
      
      // Update cargo status to 'dechargee'
      await (supabase as any)
        .from('import_cargaisons')
        .update({ statut: 'dechargee' })
        .eq('id', newReception.cargaison_id);
    },
    onSuccess: () => {
  ...
  return (
    <DashboardLayout 
      title="Réception des Cargaisons" 
      subtitle="Interface de déchargement des navires et enregistrement des entrées en stock."
    >
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button className="h-10 px-6 rounded-xl gap-2 shadow-lg shadow-blue-500/20 bg-blue-600 hover:bg-blue-700">
                <ClipboardCheck className="h-4 w-4" /> Confirmer une Arrivée
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-md rounded-3xl">
              <DialogHeader>
                <DialogTitle className="text-xl font-black text-slate-900 leading-tight">
                  Point de Réception <br/><span className="text-blue-600">Port de Conakry</span>
                </DialogTitle>
              </DialogHeader>
              <form className="space-y-4 pt-4" onSubmit={(e) => {
                ...
                createMutation.mutate({
                  cargaison_id: formData.get('cargaison'),
                  depot_id: formData.get('depot'),
                  quantite_recue: Number(formData.get('quantite')),
                ...
                <div className="space-y-2">
                  ...
                  <Select name="cargaison">
                    <SelectTrigger className="h-12 rounded-xl bg-slate-50 border-none">
                      <SelectValue placeholder="Choisir une cargaison..." />
                    </SelectTrigger>
                    <SelectContent>
                      {pendingCargo?.map((c: any) => (
                ...
                <div className="space-y-2">
                  ...
                  <Input name="quantite" type="number" placeholder="Volume exact déchargé" required className="h-12 rounded-xl" />
                </div>
                <div className="space-y-2">
                  <Label>Observations / Scellés</Label>
                  <Input name="notes" placeholder="Détails techniques optionnels" className="h-12 rounded-xl" />
                </div>
                <Button type="submit" className="w-full h-12 rounded-xl mt-4 bg-blue-600 hover:bg-blue-700 font-bold" disabled={createMutation.isPending}>
              ...
              {isLoading ? (
                ...
                <TableRow key={r.id} className="hover:bg-slate-50">
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-slate-400 font-medium flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {new Date(r.date_reception).toLocaleTimeString()}
                      </p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
                      <p className="font-bold text-blue-600 flex items-center gap-1.5">
                        <Ship className="h-3.5 w-3.5" /> {r.cargaison?.navire?.nom}
                      </p>
                      <p className="text-[10px] uppercase font-black tracking-widest text-slate-400">
                        {r.cargaison?.dossier?.numero_dossier}
                      </p>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="font-black text-slate-900">
                    {Number(r.quantite_recue).toLocaleString()} T
                  </TableCell>
                  <TableCell>
