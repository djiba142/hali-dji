...
import { 
  Warehouse, Plus, Search, MapPin, Gauge, Droplets,
  AlertTriangle, ArrowRight, Settings2, BarChart3
} from 'lucide-react';
...
export default function LogistiqueDepotsPage() {
  ...
  const { data: depots, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from('logistique_depots')
        .select(`
          *,
          stocks:logistique_stocks(
            quantite_disponible,
  ...
  return (
    <DashboardLayout 
      title="Gestion des Dépôts Pétroliers" 
      subtitle="Supervision de la capacité de stockage nationale et suivi des unités de stockage."
    >
      <div className="space-y-6">
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border">
          <div className="relative w-96">
            ...
            <Input 
              placeholder="Rechercher un dépôt ou une ville..." 
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button className="h-11 px-6 rounded-xl gap-2 shadow-lg shadow-emerald-500/20 bg-emerald-600 hover:bg-emerald-700">
                <Plus className="h-4 w-4" /> Ajouter un Dépôt
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-md rounded-3xl">
              <DialogHeader>
                <DialogTitle className="text-xl font-black">Nouveau Dépôt Pétrolier</DialogTitle>
              </DialogHeader>
              <form className="space-y-4 pt-4" onSubmit={(e) => {
                ...
                createMutation.mutate({
                  ...
                  localisation: formData.get('ville'),
                  capacite_max: Number(formData.get('capacite')),
                });
              }}>
                <div className="space-y-2">
                  <Label>Nom du Dépôt</Label>
                  <Input name="nom" placeholder="Ex: SGP Kaloum, Dépôt Kamsar..." required />
                </div>
                <div className="space-y-2">
                  <Label>Localisation (Ville)</Label>
                  <Input name="ville" placeholder="Ex: Conakry, Kankan..." required />
                </div>
                <div className="space-y-2">
                  <Label>Capacité de Stockage Totale (MT)</Label>
                  <Input name="capacite" type="number" placeholder="Ex: 500000" />
                </div>
                <Button type="submit" className="w-full h-12 rounded-xl mt-4 bg-emerald-600 hover:bg-emerald-700 font-bold" disabled={createMutation.isPending}>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {isLoading ? (
            ...
            const totalStored = depot.stocks?.reduce((acc: number, s: any) => acc + Number(s.quantite_disponible), 0) || 0;
              ...
              <Card key={depot.id} className="border-none shadow-lg hover:shadow-2xl transition-all duration-500 group overflow-hidden">
                <CardHeader className="pb-4 relative bg-slate-50/50">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <Badge className="bg-emerald-500/10 text-emerald-600 border-none text-[8px] font-black uppercase tracking-tighter">
                      Opérationnel
                    </Badge>
                  </div>
                ...
                <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="space-y-4">
                    {depot.stocks?.length > 0 ? depot.stocks.map((s: any) => (
                  ...
                  <div className="pt-2 border-t mt-4">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <Badge variant="outline" className={cn(
                        "text-[10px] h-6 px-2 font-black border-none",
                        occupancy > 80 ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600"
                      )}>
                        {occupancy.toFixed(0)}%
                      </Badge>
                    </div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button variant="outline" size="sm" className="flex-1 h-9 rounded-lg gap-1.5 font-bold text-[10px] uppercase">
