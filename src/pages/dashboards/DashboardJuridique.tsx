...
import { 
  ...
  Plus, Building2, Loader2, FolderOpen, Ship
} from 'lucide-react';
...
export default function DashboardJuridique() {
  ...
  // Fetch stations awaiting legal validation (from DLA)
  ...
  const handleApproveImport = async (dossierId: string) => {
    try {
      ...
      toast({ title: "Dossier Validé", description: "Le dossier est transmis à la DAF pour paiement." });
      refetchImports();
    } catch (err) {
    ...
    >
      ...
      <div className="mb-8 p-4 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-indigo-700 font-medium">Conformément à l'organigramme national, aucun dossier administratif ou contrat ne peut être validé par la DAF ou la DRH sans l'aval préalable de la DJ/C.</p>
        </div>
        <Button 
          size="sm" 
          variant="outline" 
      ...
      <Tabs defaultValue="analyse" className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" className="h-11 w-11 p-0 rounded-xl border-slate-200 bg-white" onClick={() => refetch()}>
                    ...
                    {isLoading ? (
                      ...
                      <tr>
                        <td colSpan={5} className="py-20 text-center">
                          ...
                          <p className="text-sm font-bold text-slate-400 uppercase">Aucune station en attente d'analyse</p>
                        </td>
                      </tr>
                    ) : (
                          ...
                          <td className="py-6 px-8 text-right">
                            <div className="flex items-center justify-end gap-2">
                               <Button 
                                ...
                                onClick={() => toast({ title: "Accès au Dossier", description: `Ouverture sécurisée du dossier ${s.code}` })}
                               >
          ...
          <div className="space-y-6 py-6">
            <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-4">
               ...
               <p className="text-xs text-slate-600 leading-relaxed">
                 Aucune entreprise ne peut obtenir de quota d'importation sans un Agrément d'Exploitation valide, certifié par la DJ/C. Le système SIHG bloque automatiquement toute attribution en cas d'expiration.
               </p>
            </div>
            <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-4">
